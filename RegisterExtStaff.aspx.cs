...
public partial class RegisterExtStaff : System.Web.UI.Page
{
    ...
    protected void ddl_ExtStaffSelect_SelectedIndexChanged(object sender, EventArgs e)
    {
        DropDownList me = (DropDownList)sender;

        string email = me.SelectedValue;

        ExternalConferenceUser user = new ExternalConferenceUser(email);
        Session["extStaffUser"]= user;
        loadForm();
        up_Transportation.Update();
    }
    ...
    private void HandleCustomPostbackEvent(string ctrlName, string args)
    {
        ...
        if (args == "OnBlur")
        {
            switch (ctrlName)
            {
                ...
                case ("tb_StaffRole"):
                    {
                        if (((ExternalConferenceUser)rsvp.User).StaffRole != tb_StaffRole.Text)
                        {
                            ((ExternalConferenceUser)rsvp.User).StaffRole = tb_StaffRole.Text;
                            //tb_LastName.Focus();
                        }
                        break;
                    }
    ...
    protected void tb_UtilityModalEntry_Validate()
    {
        if (hdn_UtilityModalPurpose.Value == "adminvite")
        {
            try
            {
                ExternalConferenceUser user = new ExternalConferenceUser(tb_UtilityModalEntry.Text);
                Session["extStaffUser"]= null;
                if (user.IsInvitee)
                    Session["extStaffUser"]= user;
            ...
            case ("submit"):
                {
                    ...
                    ConferenceUser temp = (ConferenceUser)Session["user"];
                    Session["user"] = Session["extStaffUser"];
                    writeRSVP();
//                    sendStaffConfirmationEmail(true);
    ...
    protected void rbl_TransporationOptions_SelectedIndexChanged(object sender, EventArgs e)
    {
        ...
        {
            rsvp = new RSVP(setUser(), "ExternalStaff");
        }
        else
    ...
    private ExternalConferenceUser setUser()
    {
        ExternalConferenceUser user = null;

        if (Session["extStaffUser"]== null)
        {
            string email = ddl_ExtStaffSelect.SelectedValue;

            user = new ExternalConferenceUser(email);
        ...
        {
            user = (ExternalConferenceUser)Session["extStaffUser"];
        }
    ...
    {
        ExternalConferenceUser user = (ExternalConferenceUser)Session["extStaffUser"];

        RSVP rsvp = null;

        if (Session["rsvp"] == null)
            rsvp = new RSVP(user, "ExternalStaff");
        else
            rsvp = (RSVP)Session["rsvp"];
        ...
        lblName.Text = ((ExternalConferenceUser)Session["extStaffUser"]).FullName;
        ...
        //if (!rsvp.IsNew)
        {
            ...
            tb_StaffRole.Text = user.StaffRole;
    ...
    private void updateTransportation(RSVP rsvp)
    {
