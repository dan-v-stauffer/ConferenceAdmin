...
public partial class ManageAttendee: System.Web.UI.Page
{
...
//        if(Request.LogonUserIdentity
        ...
        if (!IsPostBack)
        {
            ...
            KTConferenceUser user = new KTConferenceUser(Request.QueryString["userEmail"]);
            Session["user"] = user;
...
//                    sendStaffConfirmationEmail(true);
    ...
    protected void bn_SuccessSpeakerAddOn_Click(object sender, EventArgs e)
    {
        throw new NotImplementedException("need to assign bn_SuccessSpeakerAddOn_Click event.");
    }
    ...
    protected void upload_SpeakerPhoto_UploadComplete(object sender, AjaxControlToolkit.AjaxFileUploadEventArgs e)
    {
        KTConferenceUser user = (KTConferenceUser)Session["user"];
        string ext = string.Empty;
        for (int i = e.FileName.Length; i >= 0; i--)
        {
            if (e.FileName.Substring(i, 1) == ".")
                break;
            else
                ext = e.FileName.Substring(i, 1) + ext;
        }
        string filePath = "~/Photos/" + user.Login + "." + ext;

        upload_SpeakerPhoto.SaveAs(filePath);
    ...
    {
        DataTable days =  WebDataUtility.Instance.webAppTable("sp_GetConferenceDateRange",
            new GenericCmdParameter[] { new GenericCmdParameter("@conferenceID", Conference.Instance.ID) });

        dl_AssignEvent_Days.DataSource = days;
        dl_SelectedEvents_Days.DataSource = days;
        
        dl_AssignEvent_Days.DataBind();
        dl_SelectedEvents_Days.DataBind();
    }

    protected void dl_AssignedEventDays_ItemDataBound(object sender, DataListItemEventArgs e)
    {
        if (e.Item.ItemType == ListItemType.Item || e.Item.ItemType == ListItemType.AlternatingItem)
        {
            ...
            DataRowView drv = (DataRowView)e.Item.DataItem;
            DateTime conferenceDay = (DateTime)drv["conferenceDay"];
            Label lbl_ConferenceDay = (Label)e.Item.FindControl("lbl_ConferenceDay");
            lbl_ConferenceDay.Text = conferenceDay.ToLongDateString();

            DataList dl_AssignedEvent = (DataList)e.Item.FindControl("dl_AssignedEvent");

            DataTable tbl = WebDataUtility.Instance.webAppTable("sp_GetUserAssignedEventsList_LongForm",
                            new GenericCmdParameter[] { new GenericCmdParameter("@dateOfConference", conferenceDay),
                                                        new GenericCmdParameter("@userID", user.UserID) });

            dl_AssignedEvent.DataSource = tbl;
            if (tbl.Rows.Count > 0)
            {
                Label lbl_SelectedEvents_NoData = (Label)e.Item.FindControl("lbl_SelectedEvents_NoData");
                lbl_SelectedEvents_NoData.CssClass = "hidden";
            }
            dl_AssignedEvent.DataBind();


        }
    }

    protected void lb_AssignEvents_Click(object sender, EventArgs e)
    {
        throw new NotImplementedException("need to assign lb_AssignEvents_Click event.");
    }

    protected void lb_RemoveAssignedEvent_Click(object sender, EventArgs e)
    {
        throw new NotImplementedException("need to assign lb_RemoveAssignedEvent_Click event.");
    }

    protected void dl_AssignedEvent_ItemDataBound(object sender, DataListItemEventArgs e)
    {
        if (e.Item.ItemType == ListItemType.Item || e.Item.ItemType == ListItemType.AlternatingItem)
        {
            ConferenceUser user = (ConferenceUser)Session["user"];
            ...
            DataRowView drv = (DataRowView)e.Item.DataItem;
            LinkButton lb_RemoveAssignedEvent = (LinkButton)e.Item.FindControl("lb_RemoveAssignedEvent");
            lb_RemoveAssignedEvent.Attributes.Add("eventID", DBNullable.ToString(drv["eventID"]));

            Label lbl_EventTime = (Label)e.Item.FindControl("lbl_EventTime");
            Label lbl_Role = (Label)e.Item.FindControl("lbl_Role");
    ...
    protected void dl_SelectedEventDays_ItemDataBound(object sender, DataListItemEventArgs e)
    {
        if (e.Item.ItemType == ListItemType.Item || e.Item.ItemType == ListItemType.AlternatingItem)
        {
            ConferenceUser user = (ConferenceUser)Session["user"];
            ...
            DataList dl_SelectedEventParent = (DataList)e.Item.FindControl("dl_SelectedEventParent");
            DataTable tbl = WebDataUtility.Instance.webAppTable("sp_GetParentEventsWithSelectedChildrenByDateAndUser",
                new GenericCmdParameter[] { new GenericCmdParameter("@dateOfConference", conferenceDay),
                                            new GenericCmdParameter("@userID", user.UserID) });
            ...
            if (tbl.Rows.Count > 0)
            {
                Label lbl_SelectedEvents_NoData = (Label)e.Item.FindControl("lbl_SelectedEvents_NoData");
                lbl_SelectedEvents_NoData.CssClass = "hidden";
                 dl_SelectedEventParent.DataSource = tbl;
                dl_SelectedEventParent.DataBind();
            }
        }
    }

    protected void dl_SelectedEventParent_ItemDataBound(object sender, DataListItemEventArgs e)
    {
        if (e.Item.ItemType == ListItemType.Item || e.Item.ItemType == ListItemType.AlternatingItem)
        {
            ConferenceUser user = (ConferenceUser)Session["user"];
            int parentEventID = DBNullable.ToInt(((DataList)sender).Attributes["parentEventID"]);
            DataRowView drv = (DataRowView)e.Item.DataItem;
            //populate with child events having "date event" as parent.
            LinkButton lb_AssignEvent = (LinkButton)e.Item.FindControl("lb_AssignEvent");
            HiddenField hdn_EventID = (HiddenField)e.Item.FindControl("hdn_EventID");
            Label lbl_EventTime = (Label)e.Item.FindControl("lbl_EventTime");
            Label lbl_EventName = (Label)e.Item.FindControl("lbl_EventName");

            hdn_EventID.Value = DBNullable.ToString(drv["eventID"]);
            lb_AssignEvent.Attributes.Add("eventID", DBNullable.ToString(drv["eventID"]));

            lbl_EventTime.Text = DBNullable.ToString(drv["eventStart"]);
            lbl_EventName.Text = DBNullable.ToString(drv["eventText"]);

            if (DBNullable.ToInt(drv["eventChildCount"]) > 0)
            {
                RadioButtonList rbl_ChildEvents = (RadioButtonList)e.Item.FindControl("rbl_ChildEvents");

                rbl_ChildEvents.DataSource = WebDataUtility.Instance.webAppTable("sp_GetEventChildren",
                    new GenericCmdParameter[] { new GenericCmdParameter("@parentEventID", DBNullable.ToInt(drv["eventID"])) });
                rbl_ChildEvents.DataBind();
                rbl_ChildEvents.CssClass = String.Empty;
            }
    ...
    protected void rbl_ChildEvents_SelectedIndexChanged(object sender, EventArgs e)
    {
        RSVP rsvp = (RSVP)Session["rsvp"];
        RadioButtonList me = (RadioButtonList)sender;
        int parentEventID = DBNullable.ToInt(me.Attributes["parentEventID"]);
        int eventID = Convert.ToInt32(me.SelectedValue);

        rsvp.AssignEvent(parentEventID, eventID);
    }

    protected void lb_AssignEvent_Click(object sender, EventArgs e)
    {
        //get EventID from hidden field "hdn_EventID"
        //verify that time slot is available. if so, then assign event. If not, error message to user explaining
        //that time slot already booked and must be freed up first.
        LinkButton me = (LinkButton)sender;

        int eventID = DBNullable.ToInt(me.Attributes["eventID"]);
        ConferenceUser user = (ConferenceUser)Session["user"];

        //assign eventID to user
        //refresh 'Assigned Events' datalist and 'Selected Events' datalists
